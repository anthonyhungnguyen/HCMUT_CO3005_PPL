...
class StaticChecker(BaseVisitor, Utils):
    ...
    def visitFuncDecl(self, ast, c):
        ...
        if not c:
            ...
            return (Symbol(ast.name.name, MType(pr_type, self.visit(ast.returnType,c))),[False, False]) if self.visit(ast.returnType, c) is not VoidType else (Symbol(
    ...
    def visitId(self, ast, c):
        ...
        if not self.lookup(ast.name, flat_c, lambda x: x.name):
